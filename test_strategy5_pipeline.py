...
class TestPipeline:
    ...
    def __init__(self, args):
        ...
        self.results_dir = project_root / "results" / "strategy_5_learned"
    ...
    def load_configs(self):
        ...
        strategy_config_path = self.configs_dir / "strategy_5_learned.json"
    ...
    def run_step(self, step_num, script_name, step_name, extra_args=None):
        ...
        # 根据步骤添加特定参数
        if step_num in [1, 1, 3, 4]:  # step1b, step3, step4支持concurrency
            cmd.extend(['--concurrency', str(self.args.concurrency)])

        if step_num in [1, 3, 4]:  # step1b, step3, step4支持resume
            if self.args.resume:
                cmd.append('--resume')

        if step_num in [2, 3, 4, 5]:  # step2, step3, step4, step5仅处理strategy_5_learned
            cmd.extend(['--strategy', 'strategy_5_learned'])

        if step_num == 5:  # step5指定输出目录
            cmd.extend(['--output-dir', 'analysis/strategy_5_learned'])
        ...
        try:
            # 不捕获输出，让subprocess的stdout/stderr直接显示到控制台
            # 这样可以看到进度条，也避免了编码问题和多进程死锁
    ...
    def verify_outputs(self):
        ...
        verification = {
            'step1b': ['trained_W.pth', 'training_log.json'],
    ...
    def collect_metrics(self):
        ...
        if training_log_path.exists():
            with open(training_log_path, 'r', encoding='utf-8') as f:
                ...
                if 'normalization_scales' in training_log:
                    metrics['offset_thresholds'] = training_log.get('normalization_scales', {})
    ...
    def run(self):
        """执行完整测试流程"""
        print(f"\n{'#'*80}")
        print(f"# Strategy 5 完整测试流程")
        print(f"# 开始时间: {self.report['start_time']}")
        print(f"{'#'*80}\n")

        # 运行各个步骤
        steps = [
            (1, 'step1b_train_learned_directions.py', '训练可学习方向矩阵'),
        ...
        for step_num, script_name, step_name in steps:
            # 构建step标签（step1b特殊处理）
            step_label = 'step1' if step_num == 1 and 'b' not in script_name else \
                        'step1b' if 'b' in script_name else f'step{step_num}'
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Strategy 5 完整测试流程',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 完整测试
  python test_strategy5_pipeline.py

  # 快速测试模式
  python test_strategy5_pipeline.py --test-mode

  # 支持断点续传
  python test_strategy5_pipeline.py --resume

  # 跳过某些步骤
  python test_strategy5_pipeline.py --skip-steps step1,step2

  # 并发处理
  python test_strategy5_pipeline.py --concurrency 10

  # 详细输出
  python test_strategy5_pipeline.py --verbose
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='并发处理的进程数（默认=5，对step1b/3/4有效）'
    )

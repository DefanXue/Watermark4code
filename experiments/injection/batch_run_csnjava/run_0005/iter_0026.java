public static double distance(LineSegment3D_F64 seg,  Point3D_F64 pt) {
    // Extract coordinates for clarity and reduced dereferencing
    double ax = seg.a.x;
 double ay = seg.a.y;
    double az =  seg.a.z; 
 ...
 double bx = seg.b.x;
    ...
    double px = pt.x;
 double py = pt.y;
    double pz = pt.z;  // Initialize variables

 // Vector from segment start (a) to point (pt)
 double vecAPtX = (((px - ax))); // Check boundary conditions
 ...
 // Vector from pt to P_closest: (pt - A) - t * (B - A)
 // This is vecAPt - t * vecSeg
        double dx =  vecAPtX -  t *  vecSegX; 
        double dy =  vecAPtY -  t *  vecSegY;   // Check boundary conditions
        double dz = vecAPtZ - t * vecSegZ;  // Helper method
          return Math.sqrt(dx * dx + dy * dy + dz * dz);
 }  // Update counter
}
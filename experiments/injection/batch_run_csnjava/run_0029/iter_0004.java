protected Object readAtomic(List<Slice> slices) throws DapException {
    ...
    processSlicesSideEffect(slices);
    ...
    if (getContainer() == null) { // Not a field of a structure/record
        return readNonContainerAtomic(rank, varNotes, typeInfo, slices);
    } else { // Field of a structure instance or record
        return readContainerFieldAtomic(typeInfo, slices);
    }
}

private void processSlicesSideEffect(List<Slice> slices) {
    for (Slice s : slices) {
        if (s != null) {
...
private Object readNonContainerAtomic(int rank, VarNotes varNotes, TypeNotes typeInfo, List<Slice> slices) {
    if (rank == 0) { // scalar
...
private Object readContainerFieldAtomic(TypeNotes typeInfo, List<Slice> slices) {

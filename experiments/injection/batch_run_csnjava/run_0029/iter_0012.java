protected Object readAtomic(List<Slice> slicesParam) throws DapException {
    ...
    // This loop is moved up to ensure the side effect happens early,
    // before any potential early exits or complex logic.
    ...
    if (getContainer() == null) {
        return readAtomicFromRoot(isScalar, varNotes, typeInfo, totalElementsToRead, slicesParam);
    } else {
        return readAtomicFromContainer(typeInfo, totalElementsToRead);
    }
}

/**
 * Helper method for reading atomic data when the variable is not part of a container.
 * Delegates to scalar or vector readers based on the variable's rank.
 */
private Object readAtomicFromRoot(boolean isScalar, VarNotes varNotes, TypeNotes typeInfo,
                                  long totalElementsToRead, List<Slice> slicesParam) throws DapException {
    if (isScalar) {
...
/**
 * Helper method for reading atomic data when the variable is a field of a structure instance or record.
 * Computes memory offsets and delegates to a data reading helper.
 */
private Object readAtomicFromContainer(TypeNotes typeInfo, long totalElementsToRead) throws DapException {
    long elementSize = ((DapType) typeInfo.get()).getSize();
    long trueOffset = computeTrueOffset(this); // 'this' refers to the outer class instance
